...
###   This script attempts to download the signature file SHA256SUMS.asc from wiki.org
...
RCSUBDIR="test/"
BASEDIR="https://wiki.org/bin/"
VERSIONPREFIX="wiki-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'wiki-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the WIKI Core binary release signing key installed?"
   fi
