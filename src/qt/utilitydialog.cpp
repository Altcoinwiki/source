    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    QString version = tr("WIKI Core") + " " + tr("version") + " " + QString::fromStdString(FormatFullVersion());
    ...
    if (helpMode == about)
    {
        setWindowTitle(tr("About WIKI Core"));
        ...
        QString header = tr("Usage:") + "\n" +
            "  wiki-qt [" + tr("command-line options") + "]                     " + "\n";
        ...
        std::string strUsage = HelpMessage(HMM_WIKI_QT);
        ...
        if (showDebug) {
            strUsage += HelpMessageOpt("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", WIKIGUI::DEFAULT_UIPLATFORM));
        }
...
All the WIKI in your wallet is comprised of different \"inputs\" which you can think of as separate, discrete coins.<br> \
...
These denominations are 0.01 WIKI, 0.1 WIKI, 1 WIKI and 10 WIKI -- sort of like the paper money you use every day.</li> \
    ...
    QWidget(parent, f)
{
    ...
    layout->addWidget(new QLabel(
        tr("WIKI Core is shutting down...") + "<br /><br />" +
        tr("Do not shut down the computer until this window disappears.")));
    setLayout(layout);
}

QWidget *ShutdownWindow::showShutdownWindow(WIKIGUI *window)
{
    if (!window)
        return nullptr;
