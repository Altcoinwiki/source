...
const std::string WIKIGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString WIKIGUI::DEFAULT_WALLET = "~Default";

WIKIGUI::WIKIGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
    ...
    platformStyle(platformStyle)
{
    ...
    QPixmap pixmapLogo(":/images/light/wiki_logo_horizontal");
...
void WIKIGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a WIKI address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and wiki: URIs)"));
    ...
    aboutAction = new QAction(tr("&About WIKI"), this);
    aboutAction->setStatusTip(tr("Show information about WIKI"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for WIKI"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your WIKI addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified WIKI addresses"));
    ...
    openAction->setStatusTip(tr("Open a wiki: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the WIKI help message to get a list with possible WIKI command-line options"));
...
void WIKIGUI::createToolBars(QWidget* statusbar)
{
    if(walletFrame)
    {
...
void WIKIGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool WIKIGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool WIKIGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void WIKIGUI::setWalletActionsEnabled(bool enabled)
{
...
void WIKIGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("WIKI client") + " " + networkStyle->getTitleAddText();
...
void WIKIGUI::createIconMenu(QMenu *pmenu)
{
...
#ifndef Q_OS_MAC
void WIKIGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void WIKIGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void WIKIGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void WIKIGUI::gotoSettingsPage() 
{
    settingsAction->setChecked(true);
    if(walletFrame) walletFrame->gotoSettingsPage();
}

void WIKIGUI::gotoToolsPage()
{
    toolsAction->setChecked(true);
    if(toolsAction) walletFrame->gotoToolsPage();
}

void WIKIGUI::gotoMasternodePage()
{
    QSettings settings;
    if (settings.value("fShowMasternodesTab").toBool()) {
...
void WIKIGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void WIKIGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void WIKIGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void WIKIGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void WIKIGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to WIKI network", "", count));
    } else {
...
void WIKIGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void WIKIGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void WIKIGUI::updateHeadersSyncProgressLabel()
{
...
void WIKIGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void WIKIGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    if(!clientModel)
        return;
...
void WIKIGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("WIKI"); // default title
...
void WIKIGUI::changeEvent(QEvent *e)
{
...
void WIKIGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void WIKIGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void WIKIGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(WIKIUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void WIKIGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void WIKIGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool WIKIGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool WIKIGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void WIKIGUI::setHDStatus(int hdEnabled)
{
    QString theme = GUIUtil::getThemeName();
...
void WIKIGUI::setEncryptionStatus(int status)
{
    QString theme = GUIUtil::getThemeName();
    switch(status)
    {
...
void WIKIGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void WIKIGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void WIKIGUI::detectShutdown()
{
...
void WIKIGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void WIKIGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void WIKIGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(WIKIGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void WIKIGUI::subscribeToCoreSignals()
{
...
void WIKIGUI::unsubscribeFromCoreSignals()
{
...
void WIKIGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void WIKIGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<WIKIUnits::Unit> units = WIKIUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const WIKIUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(WIKIUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    Q_FOREACH(WIKIUnits::Unit u, WIKIUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(WIKIUnits::name(u)), this);
...
{
    setText(WIKIUnits::name(newUnits));
}
