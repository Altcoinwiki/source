...
CBlockTemplate* CreateNewBlock(const CChainParams& chainparams, const CScript& scriptPubKeyIn)
{
...

    {
        ...
        // NOTE: unlike in wiki, we need to pass PREVIOUS block height here
...
// ***TODO*** ScanHash is not yet used in WIKI
//
...
// ***TODO*** that part changed in wiki, we are using a mix with old one here for now
void static WIKIMiner(const CChainParams& chainparams, CConnman& connman)
{
    LogPrintf("WIKIMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("wiki-miner");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("WIKIMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("WIKIMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("WIKIMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("WIKIMiner -- runtime error: %s\n", e.what());
        return;
    }
}

void GenerateWIKIs(bool fGenerate, int nThreads, const CChainParams& chainparams, CConnman& connman)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WIKIMiner, boost::cref(chainparams), boost::ref(connman)));
}
