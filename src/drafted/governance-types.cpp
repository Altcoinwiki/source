...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "WIKINetwork") type = WIKINetwork;
    if(strType == "WIKINetworkVariable") type = WIKINetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == WIKINetwork) s = "WIKINetwork";
    if(type == WIKINetworkVariable) s = "WIKINetworkVariable";
