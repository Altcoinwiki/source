...
const char * const WIKI_CONF_FILENAME = "wiki.conf";
const char * const WIKI_PID_FILENAME = "wikid.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "wiki" is a composite category enabling all WIKI-related debug output
            if(ptrCategory->count(string("wiki"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\WIKICore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\WIKICore
    // Mac: ~/Library/Application Support/WIKICore
    // Unix: ~/.wikicore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "WIKICore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/WIKICore";
#else
    // Unix
    return pathRet / ".wikicore";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", WIKI_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty wiki.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override wiki.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", WIKI_PID_FILENAME));
